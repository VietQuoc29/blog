...
function BlogPost() {
  ...
  // Tự động cuộn lên đầu trang khi ID bài viết thay đổi (khi click vào bài liên quan)
  useEffect(() => {
  ...
  // LOGIC BÀI VIẾT LIÊN QUAN:
  // 1. Gắn thêm id cho data gốc
  // 2. Lọc bỏ bài viết hiện tại (p.id !== id)
  // 3. Lấy 3 bài đầu tiên (.slice)
        ...
        <div className="w-full border-b border-[#ebebeb]">
          <div className="max-w-[1128px] mx-auto px-4 md:px-6 h-[52px] md:h-[64px] flex items-center justify-between">
            <Link to="/" className="flex items-center no-underline group">
              <img src="https://static.licdn.com/aero-v1/sc/h/6zm111mce7vohqze950ilreo4" alt="LinkedIn" className="h-[21px] md:h-[26px]" />
        ...
        <nav className="hidden md:block border-b border-[#ebebeb]">
          <div className="max-w-[1128px] mx-auto px-6 h-[48px]">
            <ul className="flex space-x-10 h-full">
              {['Product updates', 'Member stories', 'Creators', 'Platform Information', 'Trust and Safety'].map(name => (
                <li key={name} className="h-full">
                  {/* BẮT BUỘC: Đính kèm biến tag về trang chủ */}
                  <Link 
                    to="/" 
                    search={{ tag: name }} 
        ...
        <article className="max-w-[768px] mx-auto px-4 md:px-6 pt-10 md:pt-16">
          ...
          <div className="flex flex-wrap items-center justify-between mb-8 gap-4">
            <div className="text-[14px] text-gray-500 flex items-center gap-2">
              {currentPost.author && (
                <>
                  <span className="font-semibold text-gray-900">{currentPost.author}</span>
                  <span>|</span>
                </>
              )}
              ...
              {relatedPosts.map((post) => (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1 max-w-[450px]">
                    {/* Tag (Màu tím giống trong ảnh) */}
                    {post.tag && post.tag !== 'Product updates' && post.tag !== 'Member stories' && (
                    ...
                    {/* Tác giả & Ngày tháng được đẩy xuống cuối */}
          ...
          <ul className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[12px] font-semibold text-[#666666]">
            ...
            {[
              ...
              <li key={item.name} className="whitespace-nowrap">
                <Link to={item.path} className="hover:text-[#0a66c2] hover:underline transition-colors text-[#666666]">{item.name}</Link>
              </li>
            ))}
          </ul>
        </div>
